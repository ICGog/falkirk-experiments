...
class boxplotter(object):
    ...
    def draw_on(self,
        ...
        ax.broken_barh([(index - w2, width)],
                       ...
                       facecolor=box_interior_color,
                       edgecolor=box_color,
                       lw=blw)
        ax.broken_barh([(index - w2, width)], (self.median, 0),
                       facecolor="white",
                       edgecolor=median_color,
                       lw=mlw)
        if self.whisk_top is not None:
            ax.broken_barh([(index - w2, width)], (self.whisk_top, 0),
                           facecolor="white",
        ...
        if self.whisk_bott is not None:
            ax.broken_barh([(index - w2, width)], (self.whisk_bott, 0),
                           facecolor="white",
                           edgecolor=whisker_color,
                           lw=blw)
            ax.broken_barh([(index, 0)],
        ...
        if self.extreme_top is not None:
            ax.scatter([index], [self.extreme_top],
                       marker='*',
                       edgecolor=box_color,
                       facecolor=box_color,
                       lw=blw)
...
def percentile_box_plot(ax,
    ...
    for index, x in indexed_data:
        ...
        bp = boxplotter(scoreatpercentile(x, 50),
                        ...
                        get_whisk(x,
                                  whisker_top), get_whisk(x, whisker_bottom),
